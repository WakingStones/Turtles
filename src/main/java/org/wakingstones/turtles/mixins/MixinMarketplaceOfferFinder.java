...
public abstract class MixinMarketplaceOfferFinder {
    ...
    @ModifyConstant(remap = false, method = "findAvailableForLevel")
    private static String fixFindAvailableForLevel(String input) {
        return "SELECT MIN(moo.price) AS price, MIN(moo.level) as level, moo.type FROM ( SELECT ca.level AS LEVEL, mo.price AS price, ca.type_id AS TYPE FROM marketplace_offers mo LEFT JOIN cards ca ON ca.id = mo.card_id WHERE ca.level = ? AND mo.seller_id != ? AND (mo.expires_at IS NULL OR mo.expires_at > ?)) AS moo GROUP BY moo.type";
    }
